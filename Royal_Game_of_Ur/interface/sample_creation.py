...
from simulator.default_settings import DEFAULT_N_STONES
...
def process_games(samples, settings, params, state):
    ...
    # print('shapes:', len(board), len(options), len(idx), len(winner))
    n_games = len(board)
    ...
    for i in range(n_games):
        ...
        if current_player == 1:
            _b = flip_board(_b)
            _o = flip_indices(_o)
            _o = np.where(_o == 31, -1, _o)  # -1 (padding) gets flipped to 31
            _i = flip_indices(_i)

        has_won = (winner[i] == current_player)
        R = 1.0 if has_won else -1.0
        ...
        # print(f'sample {i}: _b={_b.shape}, _o={_o.shape}, _i={_i.shape}, target={target.shape}')
...
def update_sample_use_count(settings, rb, used_sample_ids, sids, mids):
    ...
    for sid in used_sample_ids:
        ...
        if sid not in settings['workings']['sample_use']:
            ...
            try:  # assume average length of 200
                settings['workings']['sample_use'][sid] = \
                    np.zeros(len(rb[sid][0]), dtype=int)

            except KeyError:
                settings['workings']['sample_use'][sid] = \
                    np.zeros(max(move_ids.max()+1, 200), dtype=int)
    ...
    if first_idx - last_saved_idx > 500:
